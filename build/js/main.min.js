...
}), document.querySelector(".btn-toggler")) {
  document.querySelector(".btn-toggler").addEventListener("click", function () {
    document.querySelector(".line-buttons").classList.toggle("line-buttons--show");
  });
}
...
if (lightGallery(document.querySelector(".certificates-slider"), {
  selector: ".js_gal",
  thumbnail: !0,
  animateThumb: !0,
  ...
  new Swiper(".service-content-slider", {
    ...
    pagination: {
      ...
      renderBullet: function renderBullet(e, t) {
        return "<a href=\"#\" class=\"".concat(t, "\">").concat(e + 1, "</a>");
      }
    }
  });
}
  ...
  _createClass(ItcTabs, [{
    ...
    value: function show(e) {
      var t = this._elPanes[e.dataset.index],
        ...
        r = this._elTabs.querySelector(".tabs__pane--show");
      e !== s && (s && s.classList.remove("tabs__btn--active"), r && r.classList.remove("tabs__pane--show"), e.classList.add("tabs__btn--active"), t.classList.add("tabs__pane--show"), this._elTabs.dispatchEvent(this._eventShow), e.focus());
    }
  }, {
    key: "showByIndex",
...
document.querySelectorAll(".tariff-btn").forEach(function (e, t) {
  e.addEventListener("click", function () {
    ...
    "Что входит" == t.textContent ? t.textContent = "Свернуть" : t.textContent = "Что входит";
  });
}), document.querySelectorAll(".box-case").forEach(function (e, t) {
  var s = e.clientHeight,
    r = e.clientWidth;
  e.addEventListener("mousemove", function (t) {
    var i = t.layerX,
      n = "transform: perspective(500px) scale(1.1) rotateX(" + (t.layerY - s / 2) / s * -20 + "deg) rotateY(" + (i - r / 2) / r * 10 + "deg); transition: .1s ease";
    e.style.cssText = n;
  ...
  }(e.querySelectorAll(".tarrifs-container .tarrifs-table > div"));
}(document), function (e) {
  e.querySelectorAll("[data-open-form]").forEach(function (t) {
    t.addEventListener("click", function () {
      ...
      if (t) {
        ...
        var r = t.querySelector("form");
        if (r && s) {
          var _t = e.createElement("input");
          _t.setAttribute("type", "hidden"), _t.setAttribute("name", "nameForm"), _t.value = this.dataset.formTitle, r.appendChild(_t);
        }
      }
    });
  });
  ...
  e.querySelectorAll("[data-tags]").forEach(function (s) {
    s.addEventListener("click", function (s) {
      s.preventDefault();
      var r = this.dataset.id;
      $.post("/assets/templates/agelar/isTagsCases.php", {
        id: r
      }, function (s) {
        Number(s) ? window.location.href = this.href : t(r).then(function (t) {
          t = JSON.parse(t);
          var s = e.querySelector(".casesList");
          s.innerHTML = t.data;
          var i = s.nextElementSibling.querySelector(".show_more_cases");
          i.setAttribute("data-id", r), t.nextData ? i.style.display = "" : i.style.display = "none";
        });
      });
    });
  });
  ...
  s && s.addEventListener("click", function () {
    ...
    var s = this.getAttribute("data-id"),
      ...
      i = this.dataset.defaultShift;
    t(s, r).then(function (t) {
      if ("" != (t = JSON.parse(t)).data) {
        e.querySelector(".casesList").innerHTML += t.data, _this3.setAttribute("data-shift", Number(r) + Number(i)), t.nextData || (_this3.style.display = "none");
      }
    });
  });
}(document);